		...
		int timeout)
{
	...
	serv_addr.sin_addr.s_addr = remote_ip;
        ...
        if (flag < 0) {
                DbgPrint("%s", "[-] get fcntl error.\n");
                goto err;
        }
        if (fcntl(sock_fd, F_SETFL, flag | O_NONBLOCK) < 0) {
                DbgPrint("%s", "[-] set fcntl error.\n");
                goto err;
        }
	...
	if (connect(sock_fd, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr)) == -1) {	
		if (errno != EINPROGRESS)
			...
			return 0;
					...
					SO_ERROR, (char *)&error, (socklen_t *)&len) < 0)
                                	goto err;
                                if (error == 0) {
                                	if (fcntl(sock_fd, F_SETFL, flag) < 0) {
                                        	DbgPrint("%s", "[-] fcntl recover error.\n");
                                                goto err;
                                        }
                                        return sock_fd;
                                }
...
int listen_server(unsigned int port)
{
	...
	sock_fd = bind_sock(port);
	if (sock_fd == -1)
		return -1;

	ret = listen(sock_fd, MAX_LISTEN_USER);
	if (ret == -1)
		return -1;
